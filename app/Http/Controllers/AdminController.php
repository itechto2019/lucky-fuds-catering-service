...
class AdminController extends Controller
{
    ...
    {
        $rents = UserRent::with(['info', 'stock'])->where('status', 'approved')->where('status', 'extend')->where('status', 'returned')->get();
        return view('admin.inventory.approves')->with(compact(['rents']));
    }
    // extended
    public function Extends()
    {
        $rents = UserRent::with('info')->with('stock')->where('status', 'extend')->where('status', 'returned')->get();
